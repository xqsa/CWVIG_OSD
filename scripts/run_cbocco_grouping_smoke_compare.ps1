...
if (-not $BuildDir) {
    $BuildDir = Join-Path $RepoRoot "build\flyki_phase7c"
}
$PhaseDir = Join-Path $RepoRoot "results\phase7c"
...
function Reset-PhaseDir() {
    $resolvedResults = (Resolve-Path (Join-Path $RepoRoot "results")).Path
    if ((Test-Path $PhaseDir) -and -not ((Resolve-Path $PhaseDir).Path.StartsWith($resolvedResults))) {
        throw "Refusing to clean unexpected path: $PhaseDir"
    }
...
function Invoke-CBOCCRun([string]$RunDir, [string[]]$Arguments) {
    ...
    try {
        ...
        if (-not (Test-Path (Join-Path $RunDir "1.1.100.CBOG-CBD.result.txt"))) {
            throw "cbocco did not produce a result file. See $RunDir"
        }
    }
    finally {
        Pop-Location
...
Invoke-CBOCCRun `
    -RunDir (Join-Path $PhaseDir "legacy") `
    -Arguments @("1", "CBCCO", "1", "1000", "--grouping-source", "legacy_by_function", "--root", ".", "--require-full-coverage", "true")

Invoke-CBOCCRun `
    -RunDir (Join-Path $PhaseDir "completed_predicted") `
    -Arguments @("1", "CBCCO", "1", "1000", "--grouping-source", "explicit_files", "--po", $CompletedPo, "--oo", $CompletedOo, "--require-full-coverage", "true")

& $PythonProgram @PythonArgs `
    (Join-Path $RepoRoot "experiments\parse_cbocco_results.py") `
    --phase-dir $PhaseDir `
    --output (Join-Path $PhaseDir "comparison.csv") `
    --report (Join-Path $PhaseDir "phase7c_smoke_comparison.md")

Get-Content (Join-Path $PhaseDir "comparison.csv")
