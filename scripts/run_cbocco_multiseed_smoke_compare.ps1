...
if (-not $BuildDir) {
    $BuildDir = Join-Path $RepoRoot "build\flyki_phase7e"
}
$PhaseDir = Join-Path $RepoRoot "results\phase7e"
...
foreach ($Seed in @(1, 3, 5)) {
    ...
    $PredictedDir = Join-Path $SeedDir "predicted_10d"
    $CompletedPo = Join-Path $SeedDir "completed_predicted_groups.txt"
    $CompletedOo = Join-Path $SeedDir "completed_predicted_overlap.txt"
    ...
    & $PipelineExe `
        ...
        --seed $Seed `
        --contexts 3 `
        --output-dir $PredictedDir > (Join-Path $SeedDir "pipeline_stdout.txt") 2> (Join-Path $SeedDir "pipeline_stderr.txt")
    ...
    & $CoverageExe `
        ...
        --print-summary > (Join-Path $SeedDir "completion_summary.txt") 2> (Join-Path $SeedDir "completion_stderr.txt")
    ...
    & $SourceExe `
        ...
        --po $CompletedPo `
        --oo $CompletedOo `
        --print-summary > (Join-Path $SeedDir "completed_loader_summary.txt") 2> (Join-Path $SeedDir "completed_loader_stderr.txt")
    Check-Exit "grouping_source_cli seed $Seed"

    Invoke-CBOCCRun `
        -RunDir (Join-Path $SeedDir "legacy") `
        -Arguments @("1", "CBCCO", "$Seed", "1000", "--grouping-source", "legacy_by_function", "--root", ".", "--require-full-coverage", "true")

    Invoke-CBOCCRun `
        -RunDir (Join-Path $SeedDir "completed_predicted") `
        -Arguments @("1", "CBCCO", "$Seed", "1000", "--grouping-source", "explicit_files", "--po", $CompletedPo, "--oo", $CompletedOo, "--require-full-coverage", "true")
}
...
& $PythonProgram @PythonArgs `
    ...
    --seeds 1 3 5 `
    --fe-tolerance 0 `
    --comparison-output (Join-Path $PhaseDir "multiseed_comparison.csv") `
    --summary-output (Join-Path $PhaseDir "multiseed_summary.csv") `
    --report (Join-Path $PhaseDir "phase7e_report.md")
Check-Exit "analyze_cbocco_multiseed.py"

Get-Content (Join-Path $PhaseDir "multiseed_comparison.csv")
Get-Content (Join-Path $PhaseDir "multiseed_summary.csv")
