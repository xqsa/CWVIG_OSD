...
void printUsage()
{
    std::cout << "Usage: cbocco_grouping_loader_cli --func ID --root PATH "
...
int main(int argc, char **argv)
{
    try {
        ...
        const int func = std::stoi(requireArg(args, "--func"));
        const auto view = flyki::grouping::loadLegacyGroupingForFunction(func, requireArg(args, "--root"));
        ...
        std::cerr << "cbocco_grouping_loader_cli: " << error.what() << '\n';
        printUsage();
        return 1;
    }
}
